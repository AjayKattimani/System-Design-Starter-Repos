...
```bash
docker run --name fireline-pg -e POSTGRES_PASSWORD=postgres -p 5432:5432 -d pgvector/pgvector:pg16
```
...
This drops `fireline_test`, recreates it, enables the `vector` and `pgtap` extensions, and applies `migrations/001_retrieval_schema.sql`.
...
git commit -m "fix: repair Fireline retrieval schema to match the spec"
```
